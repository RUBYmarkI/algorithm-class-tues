...
def knapsack01_bf(wgt, val, W):
    ...
    bestSet = [] # 최적 조합 기록
    count = 0 # 부분 집합(조합) 번호 표시용
    
    for i in range(2 ** n): # 0 ~ 2^n - 1 모든 부분 집합 조합 탐색
        count += 1
        
        # 1. 각 조하에 대해 2진수 비트 패턴 생성 => 리스트에 역순으로 저장
        s = [0] * n # 비트스트링 초기화
        temp = i
        for j in range(n):
        ...
        print("선택 물건 인덱스:", chosen_items, " / 총무게 =", sumWgt, "/ 총가치 =", sumVal)
...
print("최대가치:", bestVal, "선택된 물건 인덱스:", bestSet)
print("="*100)
...
coins = [500, 100, 60, 50, 10, 5, 1] # 실제 최적해 : 동전 개수 3개 - (500, 1), (60, 2)
amount = 620
...
def kanpsackFrac_greedy(weights, values, W):
    ...
    items =[]
    for i in range(n):
        items.append((values[i]/weights[i], weights[i], value[i], i))
    ...
    # --- 단계 3: Greedy 채우기: O(n)---
    bestVal = 0
    bag_with_itams = [] # 가방에 채워지는 물건 기록
    
    for retio, wgt, val, idx in items: # 비율아 높은 순서부터 물건 채워넣기
        if W <= 0: # 배낭이 꽉 채워진 경우
            break
        if W >= wgt: # 물건을 통채로 넣을 수 있는 경우
            W -= wgt # 넣은 후 남은  용량 갱신
            bestVal += val # 최대 가치 증가
            bag_with_itams.append(("full", idx, wgt, val)) # 물건을 통채로 넣음
        else: # W< wgt 즉, 물건의 일부만 넣어야하는 경우
            fraction = W / wgt
            bestVal += val * fraction
            bag_with_itams.append(("part", idx, W, val * fraction)) # 물건을 일부 넣음
            W = 0 # 가방이 꽉 찬 상태
            break
    
    return bestVal, bag_with_itams

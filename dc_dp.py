...
#==================================================
# 1. 배열(arr)의 합 구하기 문제 : 억지기법 

def sum_bf(arr): # 시간 복잡도 - O(n), n = len(arr), 공간 복잡도 - O(1)
    """억기기법 - 단순 반복문"""
    total = 0
    for x in arr:
        total += x
...
def sum_dc(arr, left, right): # 시간 복잡도 - O(n), 시스템 스택 호출 - (n)
    """분할정복 기반 재귀함수"""
    # 원소가 한개인 경우 -> 이미 정복
    if left == right:
        return arr[left] # 종료조건
    ...
    # 병합(두 개의 부분 문제의 결과를 합침)
    return left_sum + right_sum

# 테스트
arr = [5, 3, 8, 4, 1, 6, 2, 7]
print("Interative Sum", sum_bf(arr))
...
def power_dc(x, n):
    ...
    if n == 1:# 종료 조건
        ...
        return x * power_dc(x * x, (n-1) // 2) # 재귀호출 - 절바 크기로 감소 - O(logn)
    
# 테스트 코드
x = 2.0
n = 10
print(f"억지기법_ x의 n 거듭제곱({x}, {n}) = {power_bf(x, n)}")
print(f"축소기법_ x의 n 거듭제곱({x}, {n}) = {power_dc(x, n)}")
print("="*100)
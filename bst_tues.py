...
class BSTNode:
    def __init__(self, key, value):
        """노드 생성: key(비교기준), value(저장 데이터), 좌 / 우 자식은 None으로 초기화."""
        self.key = key      # 키 (노드 비교 키준)
...
def insert_bst(root, node):
    # (1) 현재 BST틀가 비어있으면 새 노드가 그 트리의 루트가 됨
    if root is None :
        return node
    # (2) 동일한 키가 이미 있으면 중복 삽입을 허영하지 않으므로 기존 노드 그대로 반환
    if node.key == root.key :
        return root
    # (3) 삽입할 키가 현재의 루트의 키보다 작으면 서브트리로 재귀 삽입
    if node.key < root.key :
...
def delete_bst(root, key):
    ...
    # (2) 삭제할 노드가 오른쪽 서브트리에 있는지 검사
        ...
        # (3-1) 오른쪽 서브트리에서 가장 왼쪽 노드사 후계자(최솟값)
